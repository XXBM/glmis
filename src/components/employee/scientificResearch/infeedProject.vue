	...
	<div>
		...
		<el-dialog title="增加" v-model="showAddDialog">
			<el-form :model="addData" :rules="rules" ref="ruleAdd">
				<el-form-item
			      prop="projectFundedByPrivateSectorRank"
			      label="级别">
			      	<el-select v-model="addData.projectFundedByPrivateSectorRank" placeholder="请选择">
					    <el-option
					      v-for="item in this.projectFundedByPrivateSectorRanks"
					      :key="item.id"
			    ...
			    <el-form-item
			      prop="no"
			      label="编号">
			      <el-input v-model="addData.no" type='number'></el-input>
			    </el-form-item>
			    <el-form-item
			      prop="resource"
			      label="来源">
		...
		<el-dialog title="修改" v-model="showEditDialog">
			<el-form v-model="editData" :rules="rules" ref="ruleEdit">
				<el-form-item
		     	 prop="projectFundedByPrivateSectorRank"
		     	 label="级别">
			      	<el-select v-model="editData.projectFundedByPrivateSectorRank" placeholder="请选择">
					    <el-option
					      v-for="item in this.projectFundedByPrivateSectorRanks"
					      :key="item.id"
					      :label="item.description"
...
import msgDialog from '../../../components/common/msgDialog.vue'
	export default{
		data(){
			return{
				tableData:'',
				projectFundedByPrivateSectorRanks:'',
				currentPage:1,
				rows:10,
				tableDataTotal:'',
				showAddDialog:false,
		...
		methods:{
			getAllTableData:function(){
				var url=this.HOST+'/displayOwnProjectFundedByPrivateSector?page='+this.currentPage+'&rows='+this.rows
			...
			getAllProjectFundedByPrivateSectorRanks:function(){
				var url=this.HOST+'/findAllProjectFundedByPrivateSectorRanks'
				this.$http.get(url).then(response=>{
					this.projectFundedByPrivateSectorRanks=response.data
				}).catch(response=>{
			...
			add:function(){
				...
				this.getAllProjectFundedByPrivateSectorRanks()
			},
			saveAdd:function(ruleForm){
				this.$refs[ruleForm].validate((valid)=>{
					if (valid) {
						var url = this.HOST+'/addProjectFundedByPrivateSector'
			...
			edit:function(){
				if (this.currentRowId=='') {
					...
					this.editData.projectFundedByPrivateSectorRank=this.currentRow.projectFundedByPrivateSectorRank			     	
			...
			saveEdit:function(ruleForm){
				this.$refs[ruleForm].validate((valid)=>{
					if (valid) {
						...
						this.currentRow.projectFundedByPrivateSectorRank=this.editData.projectFundedByPrivateSectorRank			     	
			...
			remove:function(){
				if (this.currentRowId=='') {
					...
					this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
				          ...
				          var url =this.HOST+'/deleteProjectFundedByPrivateSector?ids='+this.currentRowId
