	...
	<div>
		...
		<el-pagination
	      ...
	      <el-form :model="addForm" :inline="true" :rules="rules" ref="addForm">
	      	...
	      	<el-form-item label="考勤状态" prop="presenceDescription">
	      		<el-select v-model="addForm.presenceDescription">
	      			<el-option
	      			v-for="item in presenceDescriptions"
	      			:key="item.id"
	      ...
	      <el-form :model="editForm" :inline="true" :rules="rules" ref="editForm">
	      	...
	      	<el-form-item label="考勤状态" prop="presenceDescription">
	      		<el-select v-model="editForm.presenceDescription">
	      			<el-option
	      			v-for="item in presenceDescriptions"
	      			:key="item.id"
	      			:label="item.description"
	      ...
	      <div v-for="item in currentRows">
	      	...
	      	<span slot="footer">
		 		...
		 		<el-button @click="delPresence" type="primary">删除</el-button>
			</span>
	      </el-dialog>
...
import msgDialog from "../../common/msgDialog"
	export default{
		...
		props:['presenceUrl','id'],
		watch:{
			presenceUrl:function(){
				this.findPresence()
				this.getSelections()
			}
		},
		methods:{
			findPresence(){
				var url = this.presenceUrl+"&page="+this.currentPage+"&rows="+this.pageSize
			...
			getSelections(){
				...
				var presenceDescriptionUrl = this.HOST + "/findAllPresenceDescriptions"
				this.$http.get(presenceDescriptionUrl).then(response=>{
					this.presenceDescriptions=response.data
				}).catch(errors=>{
			...
			editJudge(){
				if(this.currentRows.length==1){
					...
					this.editForm.presenceDescription=this.currentRows[0].presenceDescription.id
			...
			saveEdit(editForm){
				this.$refs.editForm.validate((valid)=>{
					if(valid){
						...
						this.currentRows[0].presenceDescription=this.editForm.presenceDescription
						var url = this.HOST + "/updatePresence"
			...
			delPresence(){
				...
				var url = this.HOST + "/deletePresence?ids="+this.ids
