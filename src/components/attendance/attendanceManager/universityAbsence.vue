<template>
	<div>
		...
		@selection-change="handleSelectionChange">
			...
			></el-table-column>
			<el-table-column
			prop="universityAbsenceDescription.description"
			label="考勤状态"
			align="center"
			></el-table-column>
			<el-table-column
		...
		<el-pagination
	      ...
	      <el-form :model="addForm" :inline="true" :rules="rules" ref="addForm">
	      	...
	      	<el-form-item label="考勤状态" prop="universityAbsenceDescription">
	      		<el-select v-model="addForm.universityAbsenceDescription">
	      			<el-option
	      			v-for="item in universityAbsenceDescriptions"
	      			:key="item.id"
	      ...
	      <el-form :model="editForm" :inline="true" :rules="rules" ref="editForm">
	      	...
	      	<el-form-item label="考勤状态" prop="universityAbsenceDescription">
	      		<el-select v-model="editForm.universityAbsenceDescription">
	      			<el-option
	      			v-for="item in universityAbsenceDescriptions"
	      			:key="item.id"
	      			:label="item.description"
	      ...
	      <div v-for="item in currentRows">
	      	...
	      	<span slot="footer">
		 		...
		 		<el-button @click="delUniversityAbsence" type="primary">删除</el-button>
			</span>
	      </el-dialog>
...
import msgDialog from "../../common/msgDialog"
	export default{
		...
		props:['universityAbsenceUrl','id'],
		watch:{
			universityAbsenceUrl:function(){
				this.findUniversityAbsence()
				this.getSelections()
			}
		},
		methods:{
			findUniversityAbsence(){
				var url = this.universityAbsenceUrl+"&page="+this.currentPage+"&rows="+this.pageSize
			...
			getSelections(){
				...
				var universityAbsenceDescriptionUrl = this.HOST + "/findAllUniversityAbsenceDescriptions"
				this.$http.get(universityAbsenceDescriptionUrl).then(response=>{
					this.universityAbsenceDescriptions=response.data
				}).catch(errors=>{
			...
			saveAdd(addForm){
				this.$refs.addForm.validate((valid)=>{
					if(valid){
						var url = this.HOST + "/addUniversityAbsence"
			...
			editJudge(){
				if(this.currentRows.length==1){
					...
					this.editForm.universityAbsenceDescription=this.currentRows[0].universityAbsenceDescription.id
			...
			saveEdit(editForm){
				this.$refs.editForm.validate((valid)=>{
					if(valid){
						...
						this.currentRows[0].universityAbsenceDescription=this.editForm.universityAbsenceDescription
						this.currentRows[0].remarks=this.editForm.remarks
						var url = this.HOST + "/updateUniversityAbsence"
			...
			delUniversityAbsence(){
				var i=0;
				this.currentRows.forEach(item=>{
					this.ids[i]=item.id
					i++
				})
				var url = this.HOST + "/deleteUniversityAbsence?ids="+this.ids
