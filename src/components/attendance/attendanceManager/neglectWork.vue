<template>
	<div>
		...
		@selection-change="handleSelectionChange">
			...
			></el-table-column>
			<el-table-column
			prop="neglectWorkDescription.description"
			label="考勤状态"
			align="center"
			></el-table-column>
			<el-table-column
		...
		<el-pagination
	      ...
	      <el-form :model="addForm" :inline="true" :rules="rules" ref="addForm">
	      	...
	      	<el-form-item label="考勤状态" prop="neglectWorkDescription">
	      		<el-select v-model="addForm.neglectWorkDescription">
	      			<el-option
	      			v-for="item in neglectWorkDescriptions"
	      			:key="item.id"
	      ...
	      <el-form :model="editForm" :inline="true" :rules="rules" ref="editForm">
	      	...
	      	<el-form-item label="考勤状态" prop="neglectWorkDescription">
	      		<el-select v-model="editForm.neglectWorkDescription">
	      			<el-option
	      			v-for="item in neglectWorkDescriptions"
	      			:key="item.id"
	      			:label="item.description"
	      ...
	      <div v-for="item in currentRows">
	      	...
	      	<span slot="footer">
		 		...
		 		<el-button @click="delNeglectWork" type="primary">删除</el-button>
			</span>
	      </el-dialog>
...
import msgDialog from "../../common/msgDialog"
	export default{
		...
		props:['neglectWorkUrl','id'],
		watch:{
			neglectWorkUrl:function(){
				this.findNeglectWork()
				this.getSelections()
			}
		},
		methods:{
			findNeglectWork(){
				var url = this.neglectWorkUrl+"&page="+this.currentPage+"&rows="+this.pageSize
			...
			getSelections(){
				...
				var neglectWorkDescriptionUrl = this.HOST + "/findAllNeglectWorkDescriptions"
				this.$http.get(neglectWorkDescriptionUrl).then(response=>{
					this.neglectWorkDescriptions=response.data
				}).catch(errors=>{
			...
			saveAdd(addForm){
				this.$refs.addForm.validate((valid)=>{
					if(valid){
						var url = this.HOST + "/addNeglectWork"
			...
			editJudge(){
				if(this.currentRows.length==1){
					...
					this.editForm.neglectWorkDescription=this.currentRows[0].neglectWorkDescription.id
			...
			saveEdit(editForm){
				this.$refs.editForm.validate((valid)=>{
					if(valid){
						...
						this.currentRows[0].neglectWorkDescription=this.editForm.neglectWorkDescription
						this.currentRows[0].remarks=this.editForm.remarks
						var url = this.HOST + "/updateNeglectWork"
			...
			delNeglectWork(){
				...
				var url = this.HOST + "/deleteNeglectWork?ids="+this.ids
