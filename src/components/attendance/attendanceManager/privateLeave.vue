<template>
	<div>
		...
		@selection-change="handleSelectionChange">
			...
			></el-table-column>
			<el-table-column
			prop="privateLeaveDescription.description"
			label="考勤状态"
			align="center"
			></el-table-column>
			<el-table-column
		...
		<el-pagination
	      ...
	      <el-form :model="addForm" :inline="true" :rules="rules" ref="addForm">
	      	...
	      	<el-form-item label="考勤状态" prop="privateLeaveDescription">
	      		<el-select v-model="addForm.privateLeaveDescription">
	      			<el-option
	      			v-for="item in privateLeaveDescriptions"
	      			:key="item.id"
	      ...
	      <el-form :model="editForm" :inline="true" :rules="rules" ref="editForm">
	      	...
	      	<el-form-item label="考勤状态" prop="privateLeaveDescription">
	      		<el-select v-model="editForm.privateLeaveDescription">
	      			<el-option
	      			v-for="item in privateLeaveDescriptions"
	      			:key="item.id"
	      			:label="item.description"
	      ...
	      <div v-for="item in currentRows">
	      	...
	      	<span slot="footer">
		 		...
		 		<el-button @click="delPrivateLeave" type="primary">删除</el-button>
			</span>
	      </el-dialog>
...
import msgDialog from "../../common/msgDialog"
	export default{
		...
		props:['privateLeaveUrl','id'],
		watch:{
			privateLeaveUrl:function(){
				this.findPrivateLeave()
				this.getSelections()
			}
		},
		methods:{
			findPrivateLeave(){
				var url = this.privateLeaveUrl+"&page="+this.currentPage+"&rows="+this.pageSize
			...
			getSelections(){
				...
				var privateLeaveDescriptionUrl = this.HOST + "/findAllPrivateLeaveDescriptions"
				this.$http.get(privateLeaveDescriptionUrl).then(response=>{
					this.privateLeaveDescriptions=response.data
				}).catch(errors=>{
			...
			saveAdd(addForm){
				this.$refs.addForm.validate((valid)=>{
					if(valid){
						var url = this.HOST + "/addPrivateLeave"
			...
			editJudge(){
				if(this.currentRows.length==1){
					...
					this.editForm.privateLeaveDescription=this.currentRows[0].privateLeaveDescription.id
			...
			saveEdit(editForm){
				this.$refs.editForm.validate((valid)=>{
					if(valid){
						...
						this.currentRows[0].privateLeaveDescription=this.editForm.privateLeaveDescription
						this.currentRows[0].remarks=this.editForm.remarks
						var url = this.HOST + "/updatePrivateLeave"
			...
			delPrivateLeave(){
				...
				var url = this.HOST + "/deletePrivateLeave?ids="+this.ids
